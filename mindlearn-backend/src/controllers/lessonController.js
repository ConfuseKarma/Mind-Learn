...
export async function listLessonsByTheme(req, res) {
  ...
  const lessons = await Lesson.findAll({ where: { ThemeId: id }, attributes: ['id', 'title', 'description', 'difficulty'] })
  res.json(lessons)
}

export async function getLesson(req, res) {
  const id = Number(req.params.id)
  const lesson = await Lesson.findByPk(id, { include: { model: Question, include: { model: Option, attributes: ['id', 'text'] } } })
...
export async function attemptLesson(req, res) {
  ...
  if (score === total && total > 0) {
    ...
    if (!b) b = await Badge.create({ code: 'PERFECT_SCORE', name: 'Perfect Score', description: 'Achieved 100% in a lesson' })
  ...
  if (passed) {
    ...
    if (passedLessonIds.size >= lessonsInTheme && lessonsInTheme > 0) {
      ...
      if (!b) b = await Badge.create({ code, name: `Theme ${themeId} Complete`, description: 'Completed all lessons of this theme with passing score' })
      await UserBadge.findOrCreate({ where: { UserId: req.user.id, BadgeId: b.id }, defaults: {} })
