...
async function seed() {
  ...
  const reading = await Theme.create({ name: 'Reading Basics', description: 'Foundational comprehension skills' })
  const critical = await Theme.create({ name: 'Critical Interpretation', description: 'Inference and intent' })

  const l1 = await Lesson.create({ ThemeId: reading.id, title: 'Identify subject', description: 'Understand basic sentence parts', difficulty: 1 })
  const q1 = await Question.create({ LessonId: l1.id, text: 'In the sentence "O sol nasce a leste", what is the subject?' })
  ...
  const q2 = await Question.create({ LessonId: l1.id, text: 'What is the purpose of a text conclusion?' })
  await Option.bulkCreate([
  ...
  const l2 = await Lesson.create({ ThemeId: critical.id, title: 'Main idea vs details', description: 'Identify main ideas in a paragraph', difficulty: 2 })
  const q3 = await Question.create({ LessonId: l2.id, text: 'Which sentence best states the main idea?' })
  await Option.bulkCreate([
